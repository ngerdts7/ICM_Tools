/* Trace upstream specified number of segments from selected node(s)
Travel time through each link is approximated  using the average velocity over a user-defined time window.
Reserved working fields:
Node User Number 5 => Time in minutes between this node and downstream selected node
Link User Number 4 => Time from downstream end
Link User Number 5 => Time from upstream end
Subcatchment User Number 5 => Time in minutes for this catchment flow to reach downstream selected node
*/
